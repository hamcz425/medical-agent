...
{{"verdict": "PASS", "reason": "简要说明原因"}}
或
{{"verdict": "FAIL", "reason": "简要说明原因"}}"""
...
class RAGEngine:
        ...
        try:
            ...
            try:
                ...
                if json_match:
                    ...
                    passed = verdict_data.get("verdict", "").upper() == "PASS"
                    reason = verdict_data.get("reason", verdict_text)
                else:
                    passed = False
                    reason = verdict_text
            except (json.JSONDecodeError, KeyError):
                passed = False
                reason = f"校验结果解析失败: {verdict_text}"
    ...
    async def query(
        ...
        if not response_text:
            source_titles = list({s["title"] for s in sources})
            verification = {"passed": False, "reason": "LLM服务不可用"}
            return {
                "id": 0,
                "response": None,
                "sources": sources,
