...
async def main():
    async with httpx.AsyncClient(timeout=60) as client:
        ...
        print("=== Step 1: Register doctor user ===")
        ...
        if reg_resp.status_code in (200, 201):
            ...
            print(f"  User created: doctor1 / doctor123")
        else:
            # Already exists, try login
            print(f"  Register returned {reg_resp.status_code}, trying login...")
            ...
            print(f"  Logged in as doctor1")

        headers = {"Authorization": f"Bearer {token}"}

        # Step 2: Create documents
        print(f"\n=== Step 2: Creating {len(documents)} medical documents ===")
        created_ids = []
        for i, doc in enumerate(documents, 1):
            resp = await client.post(f"{API_BASE}/documents", json=doc, headers=headers)
            if resp.status_code == 201:
                doc_id = resp.json()["id"]
                created_ids.append(doc_id)
                print(f"  [{i}/{len(documents)}] Created: {doc['title'][:30]}... (id={doc_id})")
            else:
                print(f"  [{i}/{len(documents)}] FAILED: {doc['title'][:30]}... - {resp.status_code}")

        # Step 3: Index documents one by one to avoid timeout
        print(f"\n=== Step 3: Indexing documents one by one ===")
        index_resp = await client.post(f"{API_BASE}/documents/index-all", headers=headers, timeout=300)
        if index_resp.status_code == 200:
            result = index_resp.json()
            print(f"  {result['message']}")
            for r in result.get("results", []):
                if "chunk_count" in r:
                    print(f"    {r['title'][:30]}... -> {r['chunk_count']} chunks")
                else:
                    print(f"    {r['title'][:30]}... -> ERROR: {r.get('error', 'unknown')}")
        else:
            print(f"  Index failed: {index_resp.status_code} {index_resp.text[:200]}")

        # Step 4: Verify stats
        print(f"\n=== Step 4: System Stats ===")
        stats_resp = await client.get(f"{API_BASE}/rag/stats", headers=headers)
        if stats_resp.status_code == 200:
            stats = stats_resp.json()
            print(f"  total_documents: {stats['total_documents']}")
            print(f"  indexed_documents: {stats['indexed_documents']}")
            print(f"  document_categories: {stats['document_categories']}")

        print(f"\n=== Done! ===")
        print(f"Login: doctor1 / doctor123")
        print(f"Documents created: {len(created_ids)}, IDs: {created_ids}")
